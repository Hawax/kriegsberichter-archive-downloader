...
def get_list_of_sites(_r):
    ...
    for i in range(1, 8):
        try:
            number = tree.xpath("//div[@class='navigationBar']/a[{}]".format(
                str(i)))[0].text_content()
        except IndexError:
            #print(i)
            number = tree.xpath("//div[@class='navigationBar']/a[{}]".format(
                str(i - 1)))
...
def main(_r):
    ...
    for i in range(1, 90):
        title = tree.xpath(
            "//li[{}]/div[@class='thumbnailCategory' and 1]/div[@class='description' and 2]/h3[1]/a[1]"
            .format(str(i)))
        title_url = tree.xpath(
            "//li[{}]/div[@class='thumbnailCategory' and 1]/div[@class='description' and 2]/h3[1]/a[1]"
            .format(str(i)))[0].get('href')
        title_url = 'http://www.kriegsberichter-archive.com/{}'.format(
            title_url)
        ...
        for j in range(0, how_much_sites + 1):
            print(str(j), '/', how_much_sites, '- so much left to the end ', i,
                  '- sections')

            r = open_connetion(title_url + "/" +
                               'start-{}'.format(str(j * 15)))
